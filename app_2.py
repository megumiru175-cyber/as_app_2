        ...
        try:
            ...
            for page in range(1, pages + 1):
                ...
                if "_pgn=" in url_input:
                    ...
                    connector = "&" if "?" in url_input else "?"
                    page_url = url_input + f"{connector}_pgn={page}"

                res = requests.get(page_url, headers=headers)
                if res.status_code != 200:
                    st.warning(f"ページ {page} の取得に失敗しました (status {res.status_code})")
                    continue

                soup = BeautifulSoup(res.text, "html.parser")
                items = soup.select(".s-item") if soup else []

                st.write(f"Page {page}: {len(items)} items found")
                ...
                for item in items:
                    ...
                    if title_el and price_el:
                        ...
                        # Sold / Available 判定
                        if "Sold" in sold_text or "sold" in sold_text:
                            sold = "Sold"
                        else:
                            sold = "Available"
